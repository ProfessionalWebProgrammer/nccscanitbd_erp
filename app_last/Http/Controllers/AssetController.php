...
if($i == 0){
    if($currentAmount == 0){
        ...
        $remainValue = $depData->remaining_value  + $request->total_amount - $assetSales;
    }
    $assetTotal = $depData->total_asset  + $request->total_amount - $assetSales;
    } else {
    $remainValue = $depData->total_asset - $depAmount;
    $assetTotal = $depData->total_asset;
    }
...
/*
   ...
   public function assetproduct()
    {
        $assetproduct = AssetProduct::select('asset_products.*','asset_categories.name as catname')
          ->leftJoin('asset_categories', 'asset_categories.id', 'asset_products.category_id')->get();
   ...
   public function assetproductview($id)
    ...
    public function storeassetproduct(Request $request)
    {
        ...
        $category = AssetCategory::where('id',$request->category_id)->value('name');

        $assetProduct = $this->createLedgerForCoa($category, $request->product_name);
        // if($assetProduct){
...
//  $remainValue = $request->balance - $request->dep_amount;
  ...
  public function assetlicense()
    ...
    $assetCategory = AssetProduct::select('a.id','a.name')->join('asset_categories as a', 'a.id', '=', 'asset_products.category_id')->whereNotNull('depreciation_rate')->groupBy('asset_products.category_id')->get();
