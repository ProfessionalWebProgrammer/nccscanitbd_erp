      ...
      if ($request->date) {
       ...
       /* $orderlist = DB::select('SELECT sales_orders.*,dealers.d_s_name,factories.factory_name,(select name from users where id=updated_by) as updated_by_name FROM `sales_orders`
        ...
        $orderlist = SalesOrder::selectRaw('*,ROW_NUMBER() OVER (ORDER BY id) AS row_num')
                    ...
                    ->leftJoin('users','sales_orders.emp_id','=','users.id')->where('is_active',1)->whereBetween('sales_orders.date',[$fdate,$tdate])->orderby('sales_orders.date','desc')->orderby('sales_orders.invoice_no','desc')->get(); // ->paginate(10);
      } else {
     /* $orderlist = DB::select('SELECT sales_orders.*,dealers.d_s_name,factories.factory_name,(select name from users where id=updated_by) as updated_by_name FROM `sales_orders`
        LEFT JOIN dealers ON dealers.id = sales_orders.dealer_id
        LEFT JOIN factories ON factories.id = sales_orders.warehouse_id
        ...
        */
        $orderlist = SalesOrder::selectRaw('*,ROW_NUMBER() OVER (ORDER BY id) AS row_num')->select('sales_orders.*','dealers.d_s_name','factories.factory_name')->leftJoin('dealers','sales_orders.dealer_id','=','dealers.id')
                    ->leftJoin('factories','sales_orders.warehouse_id','=','factories.id')
                    ->leftJoin('users','sales_orders.emp_id','=','users.id')->where('is_active',1)->orderby('sales_orders.date','desc')->orderby('sales_orders.invoice_no','desc')->get(); // ->paginate(10);;
      }
	//dd($orderlist);
...
//	dd($request->all());
         ...
         Sale::where('invoice_no', $request->invoice)->update([
             'is_active' => 0,
 		     ...
 		     $chalan = DB::table('chalans')->where('invoice_no',$invoice)->get();
         	if(!empty($chalan)){
           	DB::table('chalans')->where('invoice_no',$invoice)->delete();
           }
         ...
         DB::table('montly_sales_targets')->where('sales_invoice', $invoice)->delete();


        $salesReturn = DB::table('sales_returns')->where('invoice_no', $invoice)->first();
       if(!empty($salesReturn)){
       	DB::table('sales_returns')->where('invoice_no', $invoice)->delete();
       }

       $salesReturnItem = DB::table('sales_return_items')->where('invoice_no', $invoice)->first();
       if(!empty($salesReturnItem)){
       	DB::table('sales_return_items')->where('invoice_no', $invoice)->delete();
       }
