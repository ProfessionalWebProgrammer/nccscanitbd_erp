    ...
    foreach($sales as $key => $data){
      ...
      $gDTotal = 0;
     				...
     				DB::table('montly_sales_targets')->insert([
                      ...
                      $previous_ledger = SalesLedger::where('vendor_id', $data->dealer_id)->where('ledger_date', '<=', $data->date)->orderBy('ledger_date', 'desc')->orderBy('id', 'desc')->first();
                         ...
                         $ledger->save();
                             ...
                             $salesProduct = \App\Models\SalesStockIn::where('prouct_id',$val->product_id)->orderBy('id','DESC')->first();
         ...
         if($data->item_type == 'fg' ){
           ...
           $this->createDebitForFinishedGoodsSale($data->dealer_id , $gDTotal,$date,$data->narration, $invoice);
           $this->expenseSubGroupDebit('Discount ledger- dealer' , $discount,$date,$data->narration, $invoice);
         ...
         if($data->item_type == 'raw' ){
           $this->createCreditForFinishedGoodsSale('Finished Goods Sales' , $gTotal,$date,$data->narration, $invoice);
           $this->createDebitForRawMateriasSale($data->dealer_id , $gDTotal,$date,$data->narration,$invoice);
           $this->expenseSubGroupDebit('Discount ledger- dealer' , $discount,$date,$data->narration, $invoice);
    ...
    if (!$check_invoice) { */
              ...
              //  dd($sales);
                ...
                if($request->item_type[0] == 'fg' ){
                  ...
                  $this->expenseSubGroupDebit('Discount ledger- dealer' , $discount,$date,$sales->narration, $invoice);

                  $totalCostOfFg = 0;$totalRawCostOfFg = 0;
                  for($i = 0; $i < count($request->product_id); $i++){

                    $salesProduct = \App\Models\SalesStockIn::where('prouct_id',$request->product_id[$i])->orderBy('id','DESC')->first();
                    $salesProductData = \App\Models\SalesStockIn::select(DB::raw('SUM(quantity) as total_qty'),DB::raw('SUM(total_cost) as total_amount'))->where('prouct_id',$request->product_id[$i])->get();

                    if($salesProductData[0]->total_amount) {
                        $productCostBag = \App\Models\PackingConsumptions::where('pro_invoice',$salesProduct->sout_number)->whereNotNull('pro_invoice')->sum('amount');
                        if($productCostBag){
                        ...
                        $tempQty = $salesProductData[0]->total_qty + $openingData->opening_balance;
                        $tempAmount = $salesProductData[0]->total_amount + ($openingData->opening_balance * $openingData->rate);
                        //$rate = (float)$salesProduct->production_rate;
                        $rate = (float)$tempAmount/$tempQty;

                        $individualRawCost = $rate * (int) $request->delivery_qty[$i];
                        $totalCostOfFg += $individualCost;
                        $totalRawCostOfFg += $individualRawCost;
                    } else {
                        $rate = SalesProduct::where('id', $request->product_id[$i])->value('rate');
