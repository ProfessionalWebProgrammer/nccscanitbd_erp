        ...
        if ($return->save()) {
            ...
            */
            ...
            foreach ($request->products_id as $key => $item) {
                ...
                if($salesProduct) {
                  ...
                  if($checkStr == 'PRO'){
                    ...
                    foreach($rawInfos as $rawInfo){
                      ...
                      $rawStockOut->qty = -$qty;
                      $rawStockOut->rate = round($rawInfo->stock_out_rate,2);
                    ...
                    if($packingInfos){
                      foreach($packingInfos as $packingInfo){
                        ...
                        $packingStockOut->rate = round($packingInfo->rate,2);
                ...
                if($salesProduct){
                  $returnAmount = round($salesProduct->production_rate,2) * $request->p_qty[$key];
                } else {
                  $rate = \App\Models\WarehouseProduct::where('product_id',$item)->where('type','fg')->value('rate');
                  $returnAmount = round($rate,2) * $request->p_qty[$key];
                }
                    ...
                    $stockOut = new SalesStockOut();
                      ...
                      $stockOut->rate = round(($returnAmount/$request->p_qty[$key]),2);
                      $stockOut->amount = -($returnAmount);
...
public function fgReturnEdit($invoice){
      $id = Auth::id();
      $returnsdetails = SalesReturn::where('invoice_no', $invoice)->first();
      $returnsitems = SalesReturnItem::where('invoice_no', $invoice)->get();
      ...
      //dd($returnsdetails->date);
      return view('backend.sales_return.returnEdit', compact('products', 'factoryes', 'dealers', 'returnsdetails', 'returnsitems'));
    }

public function fgReturnUpdate(Request $request){
   //dd($request->all());
   if (!$request->products_id && $request->qty) {
        return redirect()->back()
            ->with('error', 'Please Insert Product');
      ...
      $userName = DB::select('select users.name from users where users.id="' . $id . '"');
      $totalAmount = 0;
      $return = SalesReturn::where('invoice_no',$invoice)->first();
      $return->total_qty = $request->grand_total_qty;
      $return->price = $request->grand_total_value;
      //$return->discount = $request->grand_total_discount;
      $return->grand_total = $request->total_payable;
      ...
      ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',18)->update([
              'debit' =>  $request->total_payable,
              'credit' => 0
              ]);
      ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',15)->update([
                      'debit' =>  0,
                      'credit' => $request->total_payable
                      ]);
                      $totalCostOfFg = 0; $totalRawCostOfFg =0;
      foreach ($request->products_id as $key => $item) {
      //  $totalAmount += $request->total_price[$key];
        $product = SalesProduct::find($item);
        $salesReturn = SalesReturnItem::where('invoice_no', $invoice)->where('product_id',$item)->first();
        $salesReturn->qty = $request->p_qty[$key];
        $salesReturn->unit_price = $request->p_price[$key];
        $salesReturn->total_price = $request->total_price[$key];
        $salesReturn->save();

        DB::table('montly_sales_targets')->where('return_invoice',$invoice)->where('product_id',$item)->update([
            'qty_kg' => -($product->product_weight * $request->p_qty[$key])
        ]);

        $ledger = SalesLedger::where('invoice',$invoice)->where('product_id',$item)->first();
        $ledger->qty_kg = -($product->product_weight * $request->p_qty[$key]);
        ...
        if($salesProduct) {
          ...
          if($checkStr == 'PRO'){
            ...
            foreach($rawInfos as $rawInfo){
              ...
              $rawStockOut = RawMaterialStockOut::where('product_id',$item)->where('invoice',$invoice)->first();
            ...
            if($packingInfos){
              ...
              /*  $packingStockOut = PackingStockOut::where('product_id',$item)->where('invoice',$invoice)->first();
                $packingStockOut->qty = -$qty;
            ...
            $rate = SalesProduct::where('id', $item)->value('rate');
            $individualCost = (int) $request->p_qty[$key] * round($rate,2);
        ...
        if($salesProduct){
          ...
          $returnAmount = round($rate,2) * $request->p_qty[$key];
              ...
              $stockOut = SalesStockOut::where('invoice',$invoice)->where('product_id',$item)->first();
              $stockOut->qty = -($request->p_qty[$key]);
              $stockOut->rate = round(($returnAmount/$request->p_qty[$key]),2);
              $stockOut->amount = -($returnAmount);
              $stockOut->status = 1;
      ...
      ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',25)->update([
              'debit' =>  $totalCostOfFg + $totalRawCostOfFg,
              'credit' => 0
              ]);
     ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',14)->update([
                              'debit' =>  0,
                              'credit' => $totalRawCostOfFg
                              ]);
    ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',163)->update([
                                'debit' =>  0,
                                'credit' => $totalCostOfFg
                                ]);


      $check_invoice = SalesLedger::where('invoice', $invoice)->where('priority', 1)->first();
      if (!$check_invoice) {
          $ledger = SalesLedger::where('invoice', $invoice)->where('priority',4)->first();
          $ledger->credit = $request->total_payable;
          $ledger->save();
    ...
    Session::put('success', 'Return Updateed Successfull. Please Check Return  List.');

   // return redirect()->route('sales.return.index')->with('success', 'Return Updateed Successfull. Please Check Return  List.');
    return redirect()->route('sales.return.index');
    ...
    public function salesReturnReport(){
      ...
      return view('backend.sales_return.salesReturnIndex', compact('categories'));
    }
    ...
    public function salesReturnReportView(Request $request){
      ...
      $dealers = SalesReturn::select('id', 'warehouse_id','dealer_id','invoice_no')
      ->where(function($query)use($category_id){
          if(!empty($category_id) && is_array($category_id))
          {
            $query->whereHas('salesReturnItems', function($query) use ($category_id) {
