...
use App\Models\JournalDebitCreditInfo;
...
// 		dd($type);
   ...
   $uid = Auth::id();
    ...
    public function journalentrycreate(Request $request)
    {
       ...
       /* $expenseLedgers = Payment::select('payments.invoice','e.id','e.subgroup_name','e.group_name')
        ...
        return view('backend.payment.journal_entry_create', compact('subgroups','subSubGroups', 'allDealers', 'allcashs', 'allBanks', 'date', 'allSuppliers','interCompany'));
    }
    ...
    //dd($request->all());
        ...
        foreach ($request->ledger_id as $key => $item) {
          ...
          if($request->dc_type == 5 || $request->dc_type == 6) {
            ...
            if ($request->dealer_id[$key] != null) {
            ...
            } elseif ($request->company_id[$key] != null) {
                $title = 'Account Payable (Intercompany)';
              if($request->dcoption2 == 1){
                $JournalEntry = new JournalEntry();
                $JournalEntry->ledger_id = $request->ledger_id[$key];
                $JournalEntry->dc_type = 1;
                $JournalEntry->user_id = Auth::id();
                $JournalEntry->credit = $request->com_credit[$key];
                $JournalEntry->subject = $request->reference;
                $JournalEntry->date = $request->date;
                $JournalEntry->save();
                $id = $JournalEntry->id+1000;
               ...
               $bankName = InterCompany::where('id',$request->company_id[$key])->value('name');
               $cash_receieve = new Payment();
               $cash_receieve->sister_concern_id = $request->company_id[$key];
               $cash_receieve->bank_name = $bankName;
               $cash_receieve->amount = $request->com_credit[$key];
               $cash_receieve->payment_date = $request->date;

               $cash_receieve->payment_type = 'PAYMENT';
               $cash_receieve->transfer_type  = 'COMPANY';
               $cash_receieve->invoice = 'Jar-'.$id;
               $cash_receieve->created_by =  $usid;
               $cash_receieve->payment_description = $request->reference ?? null;
               $cash_receieve->save();


               $this->creditJournalForAccrued($ledgerName , $request->com_credit[$key] , $request->date , $request->reference, $JournalEntry->invoice);
               $this->assetSubGroupDebit($title , $request->com_debit[$key] ,$request->date, $request->reference,$JournalEntry->invoice);

             } else {
               $JournalEntry = new JournalEntry();
               $JournalEntry->ledger_id = $request->ledger_id[$key];
               $JournalEntry->dc_type = 2;
               $JournalEntry->user_id = Auth::id();
               $JournalEntry->debit = $request->com_debit[$key];
               $JournalEntry->subject = $request->reference;
               $JournalEntry->date = $request->date;
               $JournalEntry->save();
               $id = $JournalEntry->id+1000;
              ...
              $this->debitJournalForAccrued($ledgerName , $request->com_debit[$key] , $request->date , $request->reference, $JournalEntry->invoice);
              $this->interCompanyCredit($title , $request->com_debit[$key] ,$request->date, $request->reference,$JournalEntry->invoice);
             }
	...
	public function journalEntryView($invoice){
    $journalDatas = JournalEntry::select('journal_entries.*', 'suppliers.supplier_name', 'dealers.d_s_name', 'e.subgroup_name as leg_name','e.subSubgroup_name as sub_leg_name')
            ->leftjoin('suppliers', 'suppliers.id', 'journal_entries.supplier_id')
            ->leftjoin('dealers', 'dealers.id', 'journal_entries.vendor_id')
            ->leftjoin('expanse_sub_subgroups as e', 'e.id', 'journal_entries.sub_ledger_id')
            ->where('journal_entries.invoice',$invoice)->get();
      $data = JournalEntry::where('invoice',$invoice)->first();

      $userName =  DB::table('users')->where('id', Auth::id())->value('name');
       return view('backend.payment.journal_entry_view', compact('data','journalDatas','userName'));
    }
  ...
  public function deletejournalentry(Request $request)
    {
         ...
         //dd($request->all());
          ...
          //$uid = Auth::id();
          $invoice = JournalEntry::where('invoice',$request->id)->value('invoice');

          ChartOfAccounts::where('invoice',$invoice)->delete();

          JournalEntry::where('invoice',$request->id)->delete();

         $delete = PurchaseLedger::where('invoice_no',$invoice)->delete();

        SalesLedger::where('invoice',$invoice)->delete();
        Payment::where('invoice',$invoice)->delete();
 ...
 public function generalsupplierpaymentstore(Request $request)
    ...
    public function normalJournalCreate(){
        $ledgerInfo = [];
//  $allcashs = MasterCash::orderBy('wirehouse_name', 'asc')->get();
//  $allBanks = MasterBank::orderBy('bank_name', 'asc')->get();
  $allSuppliers = Supplier::orderBy('supplier_name', 'asc')->get();
  $allDealers = Dealer::orderBy('d_s_name', 'asc')->get();
  $subgroups = ExpanseSubgroup::orderBy('subgroup_name', 'asc')->get();
  $interCompany = InterCompany::select('id','name')->get();
  /* foreach ($allcashs as $key => $value) {
    $ledgerInfo[]= $this->journalEntryLedger($value->wirehouse_id, $value->wirehouse_name);
  }
  foreach ($allBanks as $key => $value1) {
    $ledgerInfo[]= $this->journalEntryLedger($value1->bank_id, $value1->bank_name);
  } */
  foreach ($allSuppliers as $key => $value2) {
    $ledgerInfo[]= $this->journalEntryLedger($value2->id, $value2->supplier_name);
  }
  foreach ($allDealers as $key => $value3) {
    $ledgerInfo[]= $this->journalEntryLedger($value3->id, $value3->d_s_name);
  }
  foreach ($subgroups as $key => $value4) {
    $ledgerInfo[]= $this->journalEntryLedger($value4->id, $value4->subgroup_name);
  }
  foreach ($interCompany as $key => $value5) {
    $ledgerInfo[]= $this->journalEntryLedger($value5->id, $value5->name);
  }
 return view('backend.payment.normal_journal_entry_create', compact('ledgerInfo'));
    }
    
 public function normalJournalStore(Request $request)
    {
       //dd($request->all());

//dd($request->ledgerType);
        $usid = Auth::id();
        $date = $request->date;
        foreach ($request->ledger_id1 as $key => $item) {

          //$allcashs = MasterCash::select('wirehouse_name as name')where('wirehouse_name', $item)->first();
          //$allBanks = MasterBank::select('bank_name as name')where('bank_name', $item)->first();
          $supplier = Supplier::select('id','supplier_name as name')->where('supplier_name', $item)->first();
          $dealer = Dealer::select('id','d_s_name as name')->where('d_s_name', $item)->first();
        	$expenseLedger = ExpanseSubgroup::select('id','subgroup_name as name')->where('subgroup_name', $item)->first();
          $interCompany = InterCompany::select('id','name')->where('name', $item)->first();

          $item2 = $request->ledger_id2[$key];
          $supplier2 = Supplier::select('id','supplier_name as name')->where('supplier_name', $item2)->first();
          $dealer2 = Dealer::select('id','d_s_name as name')->where('d_s_name', $item2)->first();
          $expenseLedger2 = ExpanseSubgroup::select('id','subgroup_name as name')->where('subgroup_name', $item2)->first();
          $interCompany2 = InterCompany::select('id','name')->where('name', $item2)->first();
          ...
          if($request->ledgerType == 'ledger_debit'){
            ...
            $JournalEntry->user_id = Auth::id();
            $JournalEntry->debit = $request->debit[$key];
            $JournalEntry->subject = $item;
            $JournalEntry->dc_type = $request->ledgerType;
            $JournalEntry->date = $request->date;
            $JournalEntry->journel_description = $request->reference;
            $JournalEntry->save();
            $id = $JournalEntry->id+1000;
            $JournalEntry->invoice = 'Jar-'.$id;
            $JournalEntry->save();
            ...
            $JournalEntry1->user_id = Auth::id();
            $JournalEntry1->credit = $request->credit[$key];
            $JournalEntry1->subject = $request->ledger_id2[$key];
            $JournalEntry1->dc_type = 'ledger_credit';
            $JournalEntry1->date = $request->date;
            $JournalEntry1->journel_description = $request->reference;
            $JournalEntry1->invoice = $JournalEntry->invoice;
            $JournalEntry1->save();
            $this->assetSubGroupDebit($item,$request->debit[$key], $date, $request->reference, $JournalEntry->invoice );
            $this->journalEntryCredit($request->ledger_id2[$key],$request->credit[$key], $date, $request->reference, $JournalEntry->invoice );
            ...
            if($supplier){
              ...
              $ledger->journal_id = $JournalEntry->id;
              $ledger->invoice_no = $JournalEntry->invoice;
              $ledger->warehouse_bank_name = $request->ledger_id2[$key]." - "."(".$request->reference.")";
              $ledger->debit = $request->debit[$key];
              $ledger->save();

              // code for credit
              if($dealer2){
                $delaer_area_id = Dealer::where('id', $dealer2->id)->value('dlr_area_id');
                $ledger = new SalesLedger();
                $ledger->vendor_id = $dealer2->id;
                $ledger->area_id = $delaer_area_id;
                $ledger->ledger_date = $date;
                $ledger->warehouse_bank_name =  "Journal Entry";
                $ledger->invoice =  $JournalEntry->invoice;
                $ledger->narration = $request->ledger_id1[$key]." - "."(".$request->reference.")";
                $ledger->journal_id = $JournalEntry->id;
                $ledger->credit = $request->credit[$key];
                $ledger->save();
              } elseif($expenseLedger2){
                //$JournalEntryExp = new JournalEntry();
                $JournalEntry1->ledger_id = $expenseLedger2->id;
                $JournalEntry1->save();
              } else {
                $cash_receieve = new Payment();
                $cash_receieve->sister_concern_id = $interCompany2->id;
                $cash_receieve->bank_name = $interCompany2->name;
                $cash_receieve->amount = $request->credit[$key];
                $cash_receieve->payment_date = $date;
                $cash_receieve->payment_type = 'RECEIVE';
                $cash_receieve->transfer_type  = 'COMPANY';
                $cash_receieve->invoice = $JournalEntry->invoice;
                $cash_receieve->created_by =  $usid;
                $cash_receieve->payment_description = $request->reference ?? null;
                $cash_receieve->save();
              }
              ...
              //dd($dealer);
              $delaer_area_id = Dealer::where('id', $dealer->id)->value('dlr_area_id');
              $ledger = new SalesLedger();
              ...
              $ledger->warehouse_bank_name =  "Journal Entry";
              $ledger->invoice =  $JournalEntry->invoice;
              $ledger->narration = $request->ledger_id2[$key]." - "."(".$request->reference.")";
              $ledger->journal_id = $JournalEntry->id;
              $ledger->debit = $request->debit[$key];
              $ledger->save();
              ...
              if($supplier2){
                $ledger = new PurchaseLedger();
                $ledger->supplier_id = $supplier2->id;
                $ledger->date = $date;
                $ledger->journal_id = $JournalEntry->id;
                $ledger->invoice_no = $JournalEntry->invoice;
                $ledger->warehouse_bank_name = $request->ledger_id1[$key]." - "."(".$request->reference.")";
                $ledger->credit = $request->credit[$key];
                $ledger->save();
              } elseif($expenseLedger2){
              /*  $JournalEntryExp = new JournalEntry();
                $JournalEntryExp->ledger_id = $expenseLedger2->id;
                $JournalEntryExp->user_id = Auth::id();
                $JournalEntryExp->credit = $request->credit[$key];
                $JournalEntryExp->subject = $request->reference ?? 'Expense Ledger Journal Entry Credit';
                $JournalEntryExp->type = 'Expense';
                $JournalEntryExp->date = $date;
                $JournalEntryExp->invoice = $JournalEntry->invoice;
                $JournalEntryExp->journel_description = 'Expense Ledger Journal Entry Credit';
                $JournalEntryExp->save(); */
                $JournalEntry1->ledger_id = $expenseLedger2->id;
                $JournalEntry1->save();
              } else {
                $cash_receieve = new Payment();
            ...
            } elseif($expenseLedger) {
              $JournalEntry->ledger_id = $expenseLedger->id;
              $JournalEntry->save();
            /*  $JournalEntryExp = new JournalEntry();
              $JournalEntryExp->ledger_id = $expenseLedger->id;
              $JournalEntryExp->user_id = Auth::id();
              $JournalEntryExp->debit = $request->debit[$key];
              $JournalEntryExp->subject = $request->reference ?? 'Expense Ledger Journal Entry';
              $JournalEntryExp->type = 'Expense';
              $JournalEntryExp->date = $date;
              $JournalEntryExp->invoice = $JournalEntry->invoice;
              $JournalEntryExp->journel_description = 'Expense Ledger Journal Entry';
              $JournalEntryExp->save(); */
              // code for credit
              if($supplier2){
                $ledger = new PurchaseLedger();
              ...
              $cash_receieve = new Payment();
              $cash_receieve->sister_concern_id = $interCompany->id;
              $cash_receieve->bank_name = $interCompany->name;
              $cash_receieve->amount = $request->debit[$key];
              $cash_receieve->payment_date = $date;
              $cash_receieve->payment_type = 'PAYMENT';
              $cash_receieve->transfer_type  = 'COMPANY';
            ...
            //dd($request->ledgerType == 'ledger_credit');
            $JournalEntry = new JournalEntry();
            $JournalEntry->user_id = Auth::id();
            $JournalEntry->credit = $request->credit[$key];
            $JournalEntry->subject = $item;
            $JournalEntry->dc_type = 'ledger_credit';
            $JournalEntry->date = $request->date;
            $JournalEntry->journel_description = $request->reference;
            $JournalEntry->save();
            $id = $JournalEntry->id+1000;
            ...
            $JournalEntry1 = new JournalEntry();
            $JournalEntry1->user_id = Auth::id();
            $JournalEntry1->debit = $request->credit[$key];
            $JournalEntry1->subject = $request->ledger_id2[$key];
            $JournalEntry1->dc_type = 'ledger_credit';
            $JournalEntry1->date = $request->date;
            $JournalEntry1->journel_description = $request->reference;
            $JournalEntry1->invoice = $JournalEntry->invoice;
            $JournalEntry1->save();
            $this->journalEntryCredit($item,$request->debit[$key], $date, $request->reference, $JournalEntry->invoice );
            $this->assetSubGroupDebit($request->ledger_id2[$key],$request->credit[$key], $date, $request->reference, $JournalEntry->invoice );
