...
class ReceiveController extends Controller
{
    ...
    public function bankReceivecreate(Request $request)
    {
        ...
        // 28-05-2024 added options
        $subgroups = ExpanseSubgroup::all();
        $subSubGroups = DB::table('expanse_sub_subgroups')->get();

        return view('backend.receive.bankreceive', compact('allBanks', 'bk_id', 'bk_info', 'allDealers', 'date','allCompanies','category', 'subgroups', 'subSubGroups'));
    }
      ...
      public function storebankReceive(Request $request)
    {
        ...
        foreach ($request->bank_id as $key => $bankid) {
            // print_r(DB::table('expanse_subgroups')->where('id',$request->expanse_subgroup_id[$key])->value('group_id'));
            // print_r($request->expanse_subSubgroup_id[$key]);
            // exit;
            ...
            $bankdetails = MasterBank::where('bank_id', $request->bank_id[$key])->first();
              ...
              if($request->company_id[$key])
              {
                ...
                $bank_receieve->category_id = $request->category_id[$key];
                ...
                if ($bank_receieve->save()) {
                 ...
                 if($request->bank_charge[$key] > 0){
                   ...
                   $bank_charge->category_id = $request->category_id[$key];
            ...
            {
                // 28-05-2024 Expense type functionality added
                //bank will be debit here
                //ledger or sub ledger will be credit here
                ...
                if(!empty($request->expanse_subSubgroup_id[$key]))
                {
                    $expenseData = DB::table('expanse_sub_subgroups')->where('id',$request->expanse_subSubgroup_id[$key])->first();
                    $expenseDataName = $expenseData->subSubgroup_name." - ".$expenseData->subgroup_name;
                    $expenseDataGroupName = $expenseData->subSubgroup_name;
                }
                else
                {
                    $expenseData = DB::table('expanse_subgroups')->where('id',$request->expanse_subgroup_id[$key])->first();
                    $expenseDataName = $expenseData->subgroup_name." - ".$expenseData->group_name;
                    $expenseDataGroupName = $expenseData->subgroup_name;
                }
                ...
                $bank_receieve->bank_id = $request->bank_id[$key];
                $bank_receieve->bank_name = $bankdetails->bank_name;
                $bank_receieve->vendor_id = NULL;
                $bank_receieve->category_id = NULL;
                
                if($request->expanse_subgroup_id[$key])
                {
                    $bank_receieve->expanse_type_id = $expenseData->group_id;
                    $bank_receieve->expanse_subgroup_id = $request->expanse_subgroup_id[$key];
                }
                
                $bank_receieve->expanse_subSubgroup_id = $request->expanse_subSubgroup_id[$key] ?? NULL;
                $bank_receieve->amount = $request->amount[$key];
                $bank_receieve->payment_date = $request->payment_date;
                $bank_receieve->payment_type = 'RECEIVE';
                $bank_receieve->type = 'BANK';
                ...
                if($budgetminus){
                    DB::table('budget_ditributions')->where('expanse_subgroup_id',$request->expanse_subgroup_id[$key])->update([
                        'remaining_amount' => $budgetminus->remaining_amount + $request->amount[$key],
                    ]);
                }
                 ...
                 if($request->bank_charge[$key] > 0)
                {
                    ...
                    $bank_charge->bank_id = $request->bank_id[$key];
                    $bank_charge->bank_name = $bankdetails->bank_name;
                    $bank_charge->vendor_id = NULL;
                    $bank_charge->category_id = NULL;
                    
                    if($request->expanse_subgroup_id[$key])
                    {
                        $bank_charge->expanse_type_id = $expenseData->group_id;
                        $bank_charge->expanse_subgroup_id = $request->expanse_subgroup_id[$key];
                    }
                    
                    $bank_charge->expanse_subSubgroup_id = $request->expanse_subSubgroup_id[$key] ?? NULL;
                    $bank_charge->amount = $request->bank_charge[$key];
                    $bank_charge->payment_date = $request->payment_date;
                    $bank_charge->payment_type = 'EXPANSE';
                    $bank_charge->type = 'BANK';
                    ...
                    $this->expenseOnReceivedCredit($expenseDataGroupName, $request->amount[$key], $request->payment_date , $request->payment_description, $bank_receieve->invoice);
                    $this->expenseForSubSubBankCredit( $bankdetails->bank_name ,$request->amount[$key], $request->payment_date, $request->payment_description, $bank_receieve->invoice);
                    
                    $this->createCreditForBankReceive($expenseDataName, $request->amount[$key]+$bankcharge, $request->payment_date, $request->payment_description,$bank_receieve->invoice);
                    $this->createDebitForBankReceive($bankdetails->bank_name, $request->amount[$key], $request->payment_date, $request->payment_description,$bank_receieve->invoice);
                    $this->createBankChargeDebit('BANK CHARGE', $bankcharge, $request->payment_date, $request->payment_description,$bank_receieve->invoice);
                }
    ...
    public function editbankReceive($invoice) //reza
    {
        ...
        return view('backend.receive.editbankreceive', compact('bankreceivedata', 'allBanks', 'allCompanies','allDealers','bankCharge','category'));
    }
    ...
    public function updatebankReceive(Request $request)
    {
        ...
        $bank_receieve->category_id = $request->category_id;
        ...
        //dd($bank_receieve);
                ...
                $bank_charge->category_id = $request->category_id;
    ...
    public function cashReceivecreate(Request $request)
    {
        ...
        return view('backend.receive.cashreceive', compact('allcashs', 'bk_id', 'bk_info', 'allDealers', 'date','category'));
    }
    ...
    public function storecashReceive(Request $request)
    {
        ...
        foreach ($request->cash_id as $key => $cashid) {
            ...
            $cash_receieve->category_id = $request->category_id[$key];
  ...
  // return redirect()->route('cash.receive.view',$invoice);
    ...
    public function editcashReceive($invoice) // Edit Cash Received By Reza
    {
        ...
        return view('backend.receive.editcashreceive', compact('cashreceivedata', 'allcashs', 'allDealers','category'));
    }
    ...
    public function updatecashReceive(Request $request)
    {
      //  dd($request->all());
        ...
        $cash_receieve->category_id = $request->category_id;
        ...
        if ($cash_receieve->save()) {
            ...
            ChartOfAccounts::where('invoice',$request->invoice)->where('ac_sub_sub_account_id',7)->update([
                    'debit' =>  $request->amount,
                    'credit' => 0
                    ]);
        ...
        return redirect()->route('cash.receive.index')->with('success', 'Cash Received Updated Successfull');
    }
  ...
  {
    $data = Payment::select('payments.*', 'dealers.d_s_name','sc.category_name')
          ->leftjoin('dealers', 'dealers.id', 'payments.vendor_id')
          ->leftjoin('sales_categories as sc', 'sc.id', 'payments.category_id')
          ->where('status', '1');
        ...
        if ($request->bank_id && !$request->cash_id) {
            $data = $data->whereIn('payments.bank_id', $request->bank_id);
        }
        if ($request->cash_id && !$request->bank_id) {
            $data = $data->whereIn('payments.wirehouse_id', $request->cash_id);
        }
