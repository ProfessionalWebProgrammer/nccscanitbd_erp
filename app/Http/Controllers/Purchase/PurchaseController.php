...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $savePurchase = DB::table('purchases')->insertGetId([
            ...
            'inventory_amount' => ($request->receive_quantity - $request->weight_quantity) * $request->purchase_rate,
        ...
        if ($savePurchase) {
            ...
            $ledger->narration = $request->reference;
    ...
    public function update(Request $request)
    {
        ...
        $savePurchase = Purchase::where('purchase_id', $purchase_id)->update([
            ...
            'inventory_amount' => $request->receive_quantity  * $request->purchase_rate,
        ...
        if ($savePurchase) {
          ...
          $purchase =   ChartOfAccounts::where('invoice',$invoice)->where('ac_individual_account_id',114)->first();
          $purchase->debit = $request->total_payable_amount;
          $purchase->save();

          $supplier =   ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',6)->first();
          $supplier->credit = $request->total_payable_amount;
          $supplier->save();
   ...
   public function bagupdate(Request $request)
    {
       //dd($request->all());
        ...
        foreach($request->product_id as $key=>$product_ids){
          ...
          $bagPurchase = PurchaseDetail::where('purchase_id',$request->purchase_id)->where('product_id',$request->product_id[$key])->first();
          $bagPurchase->bill_quantity=$request->p_qty[$key];
          $bagPurchase->purchase_rate=$request->p_price[$key];
          $bagPurchase->amount=$request->total_price[$key];
          $bagPurchase->save();
        ...
        $invoice = Purchase::where('purchase_id',$request->purchase_id)->value('invoice');

          $bag = ChartOfAccounts::where('invoice',$invoice)->where('ac_individual_account_id',103)->first();
          $bag->debit = $totalAmount;
          $bag->save();

          $supplier = ChartOfAccounts::where('invoice',$invoice)->where('ac_sub_sub_account_id',6)->first();
          $supplier->credit = $totalAmount;
          $supplier->save();

            return redirect()->route('purchase.bag.index')->with('success', 'Bag Purchase Updated Successfully');
