...
class PurchaseLedgerController extends Controller
{
    ...
    {
        $raw_materials_report_view_fields = [
            "report_view_field_date" => $request->report_view_field_date,
            "report_view_field_invoice" => $request->report_view_field_invoice,
            "report_view_field_warehouse_bank" => $request->report_view_field_warehouse_bank,
            "report_view_field_vehicle" => $request->report_view_field_vehicle,
            "report_view_field_product" => $request->report_view_field_product,
            "report_view_field_chalan_qty" => $request->report_view_field_chalan_qty,
            "report_view_field_order_qty" => $request->report_view_field_order_qty,
            "report_view_field_receive_qty" => $request->report_view_field_receive_qty,
            "report_view_field_ded_qty" => $request->report_view_field_ded_qty,
            "report_view_field_bill_qty" => $request->report_view_field_bill_qty,
            "report_view_field_rate" => $request->report_view_field_rate,
            "report_view_field_purchase_value" => $request->report_view_field_purchase_value,
            "report_view_field_tp_fare" => $request->report_view_field_tp_fare,
            "report_view_field_debit" => $request->report_view_field_debit,
            "report_view_field_credit" => $request->report_view_field_credit,
            "report_view_field_balance_bdt" => $request->report_view_field_balance_bdt,
            "report_view_field_total_ton" => $request->report_view_field_total_ton
        ];
     ...
     if($request->type == 1){
       ...
       return view('backend.purchase_ledger.ledger_view_fg', compact('supplier','sdate','pdate', 'fdate', 'tdate','groupcount'));
        ...
        /* $ledgerspurchase = DB::table('purchase_ledgers')
              ...
              ->get(); */
              //dd($ledgerspurchase);
            return view('backend.purchase_ledger.ledger_view', compact('supplier','sdate','pdate', 'fdate', 'tdate','groupcount', 'raw_materials_report_view_fields'));
     }
...
public function ledgerReportView($fdate, $tdate){
      ...
      public function stockLedgerView($fdate,$tdate)
  {
    ...
    // 04-Jun-2024 Added
    public function rmShortSummary()
    {

        $factoryes = Factory::latest('id')->get();
        $rawProducts = RowMaterialsProduct::where('unit', '!=','PCS')->get();
        $category = SalesCategory::whereNotIn('category_name',['PACKING MATERIALS'])->orderBy('category_name', 'asc')->get();
        return view('backend.purchase_ledger.shortSummary.index', compact('rawProducts', 'factoryes', 'category'));

    }
    
    public function rmShortSummaryReports(Request $request)
    {
      ...
      if($fdate <= "2023-10-01"){
        ...
        $count = count($category);
        return view('backend.purchase_ledger.shortSummary.reports', compact('wirehousedata', 'product','fdate', 'sdate','pdate','tdate','category','count'));
    }
    // 04-Jun-2024 End
    ...
    public function bagstockLedger(Request $request)
    {
      ...
      //  dd($request->all());
        ...
        if($request->wirehouse_id){
            ...
            //$wirehousedata = Factory::latest('id')->get();
          	$wirehousedata = Factory::where('id',35)->get();
        }
