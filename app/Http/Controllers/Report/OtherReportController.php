...
class OtherReportController extends Controller
{
    ...
    public function salesAllCogsReportView(Request $request){
      ...
      if (isset($request->date)) {
       ...
       $allRawProducts = RawMaterialStockOut::where('invoice','LIKE','Sal-%')->where('status',1)->whereBetween('date',[$fdate,$tdate])->get();
       $allFgProducts = RawMaterialStockOut::where('invoice','LIKE','Sal-%')->where('status',2)->whereBetween('date',[$fdate,$tdate])->get();
       $allPackingProducts = PackingStockOut::where('invoice','LIKE','Sal-%')->where('status',1)->whereBetween('date',[$fdate,$tdate])->get();
       $rawProducts = PurchaseStockout::where('sout_number','LIKE','Sal-%')->whereBetween('date',[$fdate,$tdate])->get(); 
       ...
       return view('backend.sales_report.cogsReportView', compact('allRawProducts','rawProducts','allFgProducts','allPackingProducts','fdate','tdate','returnData'));
    }
