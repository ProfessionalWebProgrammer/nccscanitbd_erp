...
@section('content')
    ...
    font-size: 1.1rem;
                ...
                <div class="py-2 ">
                    <table class="table mt-3 " style="font-size: 14px;" id="datatablecustom">
                        <thead>
                            <tr >
                                <th >Sl No</th>
                                <th width="7.5%" >Date</th>
                                <th width="7.5%">Require Date</th>
                                <th width="7.5%">Invoice</th>
                                <th >Company</th>
                                <th >Item Name</th>
								<th>Item Code</th>
                                <th>Unit</th>
                        ...
                        <tbody style="font-size: 16px;">
                          ...
                          @foreach($reportDatas as $val)
                         @php
                          $receive =  \App\Models\MarketingOrderQc::where('invoice',$val->invoice)->first();
                          ...
                          @endphp
                              <tr>
                                  ...
                                  {{-- <td>{{date("F d, Y", strtotime($val->date))}} </td> --}}
                                    <td>{{date("d-m-Y", strtotime($val->date))}} </td>
                                    <td>{{date("d-m-Y", strtotime($val->require_date))}} </td>
                                    <td>{{$val->invoice}} </td>
                                    <td>{{$val->com->name}} </td>
                                    <td>{{$val->mp->name}} </td>
	                                	<td>{{$val->mp->code}}</td>
	                                	<td>{{$val->mp->unit}}</td>
										<td>{{ $val->mp->saleCat->category_name }}</td>
                                    <td>{{ $val->mp->subSaleCat->name }}</td>
	                                  <!-- <td><a href="{{URL::to('/public/uploads/marketing/')}}/{{$val->mp->image}}" target="_blank"><img class="gallery" style="height:50px;" src="{{URL::to('/public/uploads/marketing/')}}/{{$val->mp->image}}" alt="Image"></a></td> -->
	                                	<td>{{$val->qty}}</td>
	                                	 <td>{{$recQty}}</td>
                                    <td align="center">@if($val->status == 1) <span class="badge badge-success p-2"> Approved </span> @elseif($val->qcStatus == 0) <span class="badge badge-danger p-2"> Rejected </span> @else <span class="badge badge-info p-2"> Pending </span> @endif </td>
                                    <td align="center">@if($val->purchaseOrderStatus == 1) <span class="badge badge-success p-2">Confirm</span>@elseif($val->purchaseOrderStatus == 0) <span class="badge badge-danger p-2">Rejected </span>  @else   @endif </td>
																		<td align="center">@if($val->qcStatus == 1) <span class="badge badge-success p-2">Accept</span> @elseif($val->qcStatus == 0) <span class="badge badge-danger p-2"> Rejected </span> @else   @endif </td>
                                    <td>{{$val->user->name}}</td>
                                </tr>
                                @endforeach
                           </tbody>
    ...
    $(function () {
        $("#btnExport").click(function () {
            $("#datatablecustom").table2excel({
                filename: "marketing-order-reports.xls"
            });
        });
    });
