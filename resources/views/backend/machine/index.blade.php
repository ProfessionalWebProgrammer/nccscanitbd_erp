                    ...
                    <table id="example1" class="table table-bordered table-striped table-fixed" style="font-size: 15px;">
                        ...
                        <tbody style="font-size: 18px;">
                            ...
                            @foreach ($machine as $val)
								@php 
                          			$name = DB::table('factories')->where('id',$val->factory_id)->value('factory_name');
                          		@endphp 
                                <tr>
                                    ...
                                    <td>{{ $val->type }} </td>
                                     <td>{{$val->work }}</td>
                                    <td>{{ $val->production_per_hour }}</td>
                                    <td>{{ $val->lifecycle }} </td>
                                    <td>{{ $val->efficiency }} </td>
                                    <td>{{ $val->description }} </td>


                                    <td class="text-center align-middle">
                                       <a class="btn btn-sm salesedit" style="background-color: #66BB6A"
                                            href="{{ URL::to('machine/edit/' . $val->id) }}" data-toggle="tooltip"
