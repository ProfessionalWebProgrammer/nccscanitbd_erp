                            ...
                            <tbody style="font-size: 11px;">
                                ...
                                @php
                                  ...
                                  $regions = \App\Models\SalesLedger::select('region_id as id','t1.subzone_title as name')
                                                  ...
                                                  ->where('sales_ledgers.zone_id', $val->id)->whereIn('vendor_id',$dealer_id)
                                ...
                                @if(count($regions) > 0)
                                  @foreach ($regions as $key => $val)
                                      ...
                                      $areas = \App\Models\SalesLedger::select('sales_ledgers.area_id as id','t1.area_title as name','vendor_id')
                                                      ->leftjoin('dealer_areas as t1','sales_ledgers.area_id','=','t1.id')
                                                      ->where('sales_ledgers.region_id', $val->id)->whereIn('vendor_id',$dealer_id)
                                      ...
                                      <tr>
                                        ...
                                        $dealerDatas = \App\Models\SalesLedger::select('sales_ledgers.vendor_id as id','t1.d_s_name as name','t1.monthly_target')
                                                        ...
                                                        ->where('sales_ledgers.area_id', $val->id)->where('vendor_id',$val->vendor_id)
                                        ...
                                        @foreach ($dealerDatas as $key => $val)
                                            @php
                                              $products = \App\Models\SalesLedger::where('sales_ledgers.vendor_id', $val->id)
