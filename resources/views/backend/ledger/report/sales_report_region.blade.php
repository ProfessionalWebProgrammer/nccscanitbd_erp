                            ...
                            <tbody style="font-size: 11px;">
                                ...
                                @php
                                  ...
                                  $regions = \App\Models\SalesLedger::select('region_id as id','t1.subzone_title as name')
                                                  ...
                                                  ->where('sales_ledgers.zone_id', $valZone->id)->whereIn('region_id',$region_id)
                                ...
                                @if(count($regions) > 0)
                                  @foreach ($regions as $key => $val)
                                      ...
                                      /* $areas = \App\Models\SalesLedger::select('t1.area_title as name',DB::raw('SUM(debit) as total_debit'))
                                                      ->leftjoin('dealer_areas as t1','sales_ledgers.area_id','=','t1.id')
                                                      ->where('sales_ledgers.region_id', $val->id)
                                                      ->whereBetween('ledger_date' ,[$fdate,$tdate])->groupBy('sales_ledgers.area_id')->get(); */

                                      $areas = \App\Models\SalesLedger::select(
                                                                    't1.area_title as name','t1.id as areaId','sales_ledgers.region_id',
                                                                  ...
                                                                  ->where('sales_ledgers.zone_id', $valZone->id)->where('sales_ledgers.region_id', $val->id)->groupBy('sales_ledgers.area_id')->get();
                                      ...
                                      <tr>
                                        ...
                                        @php
                                        $openingBalance = \App\Models\SalesLedger::select(DB::raw('SUM(debit) - SUM(credit) as balance'))->where('zone_id', $valZone->id)->where('region_id', $val->region_id)->where('area_id', $val->areaId)
                                        ...
                                        @endphp
                                           ...
                                           <tr>
                                            <td>{{++$key}}</td>
                                            <td style="text-align: left; color: #8b1ba9; font-size: 13px; font-weight:500;">Area: {{$val->name ?? ''}}</td>
                                ...
                                {{-- if Region End --}}
                                 <tr style="background-color: #16c11a; font-size: 15px;">
                                  <td colspan="2" style="text-align: left;">Zone Sub Total: </td>
