            ...
            <div class="container-fluid">
                <div class="pt-3">
                  <div class="col-md-12 text-right">
                    <a href="{{route('employee.leave.of.absent.policy.create')}}" class="btn btn-sm btn-success">Employee Leave Policy Create </a>
                ...
                <div class="py-4 table-responsive">
                    ...
                    <table id="example1" class="table table-bordered table-striped" style="font-size: 15px;">
                        ...
                        <tbody>
                        @foreach($policyDatas as $val)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$val->head}}</td>
                                    <td class="text-center">{{$val->count}}</td>
                                    <td class="text-center">{{$val->fine}}</td>
                                    <td>{{$val->note}}</td>
    ...
    <div class="modal fade" id="delete">
        <div class="modal-dialog">
            <div class="modal-content bg-danger">
                ...
                <form action="{{ route('employee.leave.of.absent.policy.delete') }}" method="POST">
                    {{ method_field('delete') }}
                    @csrf
