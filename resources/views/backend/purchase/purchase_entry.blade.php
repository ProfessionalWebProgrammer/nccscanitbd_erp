            ...
            <form class="floating-labels m-t-40" action="{{ Route('purchase.store') }}" method="POST">
                ...
                <div class="container" style="background:#ffffff; padding:0px 40px;min-height:85vh">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3" id="normalPro">
                        ...
                        {{-- <div class="col-md-3" id="selectPro" style="display:none;">
                          <label style="padding-top: 10px;">Product Name</label>
                          <input type="text" class="form-control"  id="rawProduct" >
                          <input type="hidden" name="rm_product_id" id="rawProId" >
                        </div> --}}
                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-10">
                                            <div class="row">
                                                <div class="col-md-5">
                                                  <label for="">Select Aditional Supplier:</label>
                                                    <select class="form-control select2 products_id" name="ad_supplier_id[]"
                                                        data-live-search-style="startsWith" required>
                                                        <option value=" " selectedS>Select Supplier</option>
                                                        @foreach ($suppliers as $supplier)
                                                            <option style="color:#000;font-weight:600;" value="{{ $supplier->id }}">
                                                                {{ $supplier->supplier_name }}</option>
                                                        @endforeach
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <input type="text" class="form-control ad_qty" name="ad_qty[]"  placeholder="Quantity">
                                                </div>
                                                <div class="col-md-2">
                                                    <label for="">Rate :</label>
                                                    <input type="text" class="form-control ad_price" name="ad_rate[]" placeholder="Price">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="">Total :</label>
                                                    <input type="text" readonly tabIndex="-1" class="form-control ad_total_price"  name="ad_total_price[]" placeholder="total">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                    ...
                    <div class="row mt-4">
                      <div class="col-md-8">
                        ...
                        <div class="col-md-4">
                          <div class="align-right h6" id="ad_total_value">  </div>
                          <input type="hidden" id="all_total_cost" name="all_total_cost" value="">
                        </div>
                        </div>
                      </div>
                    </div>
...
$(document).ready(function() {
    ...
    $(document).on('focus', '.select2-selection.select2-selection--single', function (e) {
        ...
        $("#field").on("click", ".addMore", function() {
            x = x+1;
            var fieldHTML =
                '<div class="row fieldGroup rowname mt-2"> <div class="col-md-12"> <div class="row"> <div class="col-md-10"> <div class="row"> <div class="col-md-5">  <select class="form-control select2 products_id" name="ad_supplier_id[]" data-live-search-style="startsWith" required> <option value=" " selectedS>Select Supplier</option> @foreach ($suppliers as $supplier) <option style="color:#000;font-weight:600;" value="{{ $supplier->id }}"> {{ $supplier->supplier_name }}</option> @endforeach </select> </div> <div class="col-md-2">  <input type="text" class="form-control ad_qty" name="ad_qty[]"  placeholder="Quantity"> </div> <div class="col-md-2"> <input type="text" class="form-control ad_price" name="ad_rate[]" placeholder="Price"> </div> <div class="col-md-3"> <input type="text" readonly tabIndex="-1" class="form-control ad_total_price" name="ad_total_price[]" placeholder="total"> </div> </div> </div> <div class="col-md-2"> <a href="javascript:void(0)" style="margin-top: 3px;" class="btn custom-btn-sbms-add btn-light btn-sm addMore"><i class="fas fa-plus-circle"></i></a> <a href="javascript:void(0)" class="btn btn-danger btn-sm custom-btn-sbms-remove remove" style="margin-top: 3px;"><i class="fas fa-minus-circle"></i></a> </div> </div> </div> </div>';
        ...
        $('#field').on('input','.ad_price, .ad_qty',function(){
            var parent = $(this).closest('.fieldGroup');
            var product_price=parent.find('.ad_price').val();
            var product_qty = parent.find('.ad_qty').val();
            var total_price = product_price*product_qty;
            parent.find('.ad_total_price').val(total_price);
            total();
          });
            ...
            function total(){
              ...
              $(".ad_total_price").each(function(){
                  var totalvalueid = $(this).val()-0;
                  total +=totalvalueid;
                  $('#ad_total_value').html(total);
                  $("#all_total_cost").val(total);
              });
            ...
            $('#po_id').on('change',function(){
              var order_id = $('#po_id').val();
              //alert(order_id);
              if(order_id){
                $.ajax({
                        url : '{{url('/purchase/order/rate/')}}/'+order_id,
                        type: "GET",
                        dataType: 'json',
                        success : function(data){
                           ...
                           //console.log(data);
                           if(data.rate > 0){
                             $('#purchase_rate').val(data.rate).attr('readonly', 'readonly');
                             /*$('#rawProduct').val(data.prodcut).attr('readonly', 'readonly');
                             $('#rawProId').val(data.prodcutId);
                             $('#selectPro').css('display',"block");
                             $('#normalPro').css('display',"none"); */
                           } else {
                             /*$('#selectPro').css('display',"none");
                             $('#normalPro').css('display',"block"); */
                             $('#purchase_rate').val(data.rate).removeAttr('readonly');
                           }
