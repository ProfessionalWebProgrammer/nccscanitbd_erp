               ...
               <div class="row pt-2">
                ...
                <div class="text-center pt-3">
                    <div class="flex float-right">
                        <a href="{{ route('sub.sub.account.list') }}" class="btn btn-primary text-white">Sub Sub Account List</a>
                    </div>
                </div>
                <div class="py-4">
                    <div class="text-center">
                      <h6 class=" font-weight-bold" style="font-size:24px;">Sub Sub  Account Create</h6>
                        <hr>
                    </div>
                  
                    <form class="floating-labels m-t-40" action="{{ Route('sub.sub.account.store') }}" method="POST">
                        @csrf
                                    ...
                                    <div class="form-group  col-md-6">
                                        ...
                                        {{-- <div class="col-sm-9"> --}}
                                            ...
                                            <select class="form-control select2" style="padding: 0px 10px 10px 10;"
                                                name="ac_sub_account_id" required>
                                                  <option value="">Select Sub Account</option>
                                                @foreach ($subAccounts as $subAccount)
                                                  <option value="{{ $subAccount->id }}">{{ $subAccount->title }} ({{ $subAccount->code}})</option> 
                                                @endforeach
                                            </select>
                                        {{-- </div> --}}
                                    </div>
