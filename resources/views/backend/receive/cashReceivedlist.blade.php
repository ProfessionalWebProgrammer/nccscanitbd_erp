    ...
    @section('content')
        ...
        <div class="content-wrapper accountscontent">
            ...
            <div class="content px-4 ">
                <div class="container" style="background:#ffffff; padding:0px 40px;min-height:85vh;max-width: 1475px !important;">

               <div class="row pt-3">
                 <div class="col-md-6 text-right"></div>
                      <div class="col-md-6 text-right">
                      	 <a href="{{ URL('/cash/receive/create') }}" class=" btn btn-success mr-2">Create Cash Receive</a>
                       </div> 
                  </div>
                ...
                <div class="">
                    <div class="text-center">
                        <h5 class="text-uppercase font-weight-bold">Cash Received List</h5>
                        <hr>
                    </div>
                    <form action="{{ route('cash.receive.index') }}" method="get">
                        <div class="row pb-4">
                                ...
                                <div class="col-md-8 input-group rounded">
                                    ...
                                    <div class="input-group-prepend pr-2">
                                        <a href="{{ route('cash.receive.index') }}" class="btn btn-sm btn-danger"><i class="far fa-times-circle"></i>
                                            Clear</a>
                                    </div>
                                    {{-- <div class="input-group-prepend">
                    ...
                    <table id="example3" class="table table-bordered table-striped" style="font-size: 15px;">
                        <thead>
                            <tr class="text-center" style="background: #FA621C; color: #fff;">
                                <th>Sl</th>
                                <th>User</th>
                                <th>Depot/ WireHouse Name</th>
                                <th>Supplyer Name / Vendor</th>
                                <th>Payment Date</th>
                                <th>Invoice</th>
                                <th>Amount</th>
                                <th>Updated By</th>
                        ...
                        @foreach ($data as $item)
                            ...
                            $sl++;
                              ...
                              $updatename = DB::table('users')->where('id',$item->updated_by)->value('name');
                              $total += $item->amount;
                            @endphp
                                <tr >
                                    ...
                                    <td class="align-middle">{{$item->wirehouse_name}}</td>
                                    <td class="align-middle">{{$item->d_s_name}}</td>
                                    ...
                                    <td class="text-center align-middle">
                                        <a class="btn btn-xs text-light accountsedit" style="background-color: #66BB6A"
                                                href="{{ URL::to('/cash/receive/edit/' . $item->invoice) }}"
                                                data-toggle="tooltip" data-placement="top" title="Edit"><i
                                                    class="fas fa-edit"></i> </a>
                                      <a class="btn btn-xs text-light btn-primary" 
                                                href="{{ URL::to('/cash/receive/view/' . $item->invoice) }}"
        ...
        <div class="modal fade" id="delete">
            <div class="modal-dialog">
                <div class="modal-content bg-danger">
                    ...
                    <form action="{{ route('cash.receive.delete') }}" method="POST">
                        {{ method_field('delete') }}
                        @csrf
