        ...
        <div class="content px-4 ">
            ...
            <div class="container-fluid" id="cardbody">
                ...
                <div class="text-center pt-3 mb-4">
                    <h5 class="text-uppercase font-weight-bold">Bank Received View</h5>
                   <hr>
                </div>
                <div class="row pt-5">
                    ...
                    @php
                      ...
                      $updatename = DB::table('users')
                                ...
                                if(empty($data->d_s_name)){
                                  $name = \App\Models\InterCompany::where('id',$data->sister_concern_id)->value('name');
                                }
                                
                        $ledger = '';     
                        $subLedger = '';     
                        if(!empty($data->expanse_subgroup_id))
                        {
                            $ledger = DB::table('expanse_subgroups')->where('id', $data->expanse_subgroup_id)->first();
                            $ledger = $ledger->subgroup_name .'-'.$ledger->group_name;
                            $subLedger = DB::table('expanse_sub_subgroups')->where('id', $data->expanse_subSubgroup_id)->first();
                            $subLedger = $subLedger->subSubgroup_name .'-'. $subLedger->subgroup_name;
                        }
                    @endphp
                        ...
                        <table id="" class="table table-bordered table-striped table-fixed" style="font-size:20px;">
                            ...
                            <tbody>
                                <tr>
                                    <td rowspan="2">1</td>
                                    @if(!empty($data->expanse_subgroup_id))
                                        <td class="align-middle" colspan="2">{{ $data->bank_name }}</td>
                                        <td></td>
                                    @else
                                        <td class="align-middle" colspan="2">{{ $data->bank_name }}</td>
                                    @endif
                                ...
                                <tr>
                                    ...
                                    @if(!empty($data->expanse_subgroup_id))
                                        ...
                                        <td class="align-middle">{{$subLedger}}</td>
                                    @else
                                        <td class="align-middle" colspan="">{{ $data->d_s_name ?? $name }}</td>
                                    @endif
                        	 ...
                        	 <tr>
                            <td colspan="7" align="left" >Total Amount: {{ convert_number($data->amount).convert_paisa((string)$data->amount) }}</td>
