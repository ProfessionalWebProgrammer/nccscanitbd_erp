                        ...
                        <tbody style="font-size: 18px;">
                            ...
                            @foreach ($meter as $date)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $date->meter_no }} </td>
                                    <td>{{ $date->meter_name }} </td>
                                     <td>{{ $date->opening_reading }}</td>
                                    <td>{{ $date->producton_per_hour }}</td>
                                    <td>{{ $date->per_cycle_rotation }}</td>


                                    <td class="text-center align-middle">

                                       <a class="btn btn-sm salesedit" style="background-color: #66BB6A"
                                            href="{{ URL::to('meter/edit/' . $date->id) }}" data-toggle="tooltip"
      ...
      <div class="modal fade" id="delete">
        <div class="modal-dialog">
            <div class="modal-content bg-danger">
                ...
                <form action="{{ route('delete.meter') }}" method="POST">
                    {{ method_field('delete') }}
                    @csrf
