        ...
        <div class="content px-4 ">
            ...
            <div class="container-fluid" style="background:#ffffff; padding:0px 40px;min-height:85vh" id="contentbody">
              <div class="row pt-2">
                      ...
                      <h5 class="text-uppercase font-weight-bold">F.G Transfer Stock Report</h5>
                         ...
                         @php
                            ...
                            @endphp
                             ...
                             <tr style="font-size: 12x;"><!--  -->
                                    ...
                                    <td>{{$product_name}}</td>
                                    <td>{{$opblnce * $weight}} @if($opblnce) {{$weightUnit ?? ''}} @else  @endif </td>

                                    <td>{{$opblnce}} @if($opblnce) {{$unit ?? ''}} @else  @endif</td>
                                    <td>  {{number_format($rate,2)}} </td>
																		<td> {{number_format($openingAmount, 2)}}</td>
                                    <td>{{$todaystock[0]->quantity * $weight}} @if($todaystock[0]->quantity) {{$weightUnit ?? ''}} @else  @endif</td>
                                    <td>{{$todaystock[0]->quantity}} @if($todaystock[0]->quantity) {{$unit ?? ''}} @else  @endif</td>

                                    <td>{{($sales[0]->qty) * $weight}} @if($sales[0]->qty) {{$weightUnit ?? ''}} @else  @endif</td>
                                    <td>{{$sales[0]->qty}} @if($sales[0]->qty) {{$unit ?? ''}} @else  @endif</td>
                                    <td>@if(($sales[0]->qty) > 0){{number_format(($sales[0]->amount/($sales[0]->qty)),2)}} @else  @endif</td>
                                    <td>{{number_format($sales[0]->amount,2)}} </td>


																		<td></td>
																		<td></td>

                                    <td>{{round($returnp[0]->qty * $weight)}} @if($returnp[0]->qty) {{$weightUnit ?? ''}} @else  @endif</td>
                                    <td>{{round($returnp[0]->qty)}} @if($returnp[0]->qty) {{$unit ?? ''}} @else  @endif</td>

                                     <td>{{$transfer_to->qty * $weight }} @if($transfer_to->qty) {{$weightUnit ?? ''}} @else  @endif </td>
                                     <td>{{$transfer_to->qty}} @if($transfer_to->qty) {{$unit ?? ''}} @else  @endif </td>

                                    <td>{{$transfer_from->qty * $weight}} @if($transfer_from->qty) {{$weightUnit ?? ''}} @else  @endif</td>
                                    <td>{{$transfer_from->qty}} @if($transfer_from->qty) {{$weightUnit ?? ''}} @else  @endif</td>

																		<td>{{number_format(($damage->qty * $weight), 2)}} @if($damage->qty) {{$weightUnit ?? ''}} @else  @endif</td>
                                    <td>{{number_format($damage->qty, 2)}} @if($damage->qty) {{$unit ?? ''}} @else  @endif</td>
