                    ...
                    <table id="reporttable" class="table table-bordered table-striped table-fixed"
                        ...
                        <tbody>
                            ...
                            @endphp
                                ...
                                @foreach ($rawPurchaseDatas as $purchas_data)
                                    @php
                                        $openingQty = $purchas_data->product->opening_balance ?? 0;
                                        $purchaseStock = \App\Models\Purchase::select(DB::raw('SUM(inventory_receive) as total_stock'))->where('product_id',$purchas_data->product_id)->whereBetween('date', [$sdate, $pdate])->get();
                                        $rawStockout = \App\Models\PurchaseStockout::select(DB::raw('SUM(stock_out_quantity) as total_stockOut'))->where('product_id',$purchas_data->product_id)->whereBetween('date', [$sdate, $pdate])->get();
                                        
                                        if($purchaseStock){
                                           $stockIn = $purchaseStock[0]->qty; 
                                           } else {
                                           $stockIn = 0; 
                                           }
                                           if($rawStockout){
                                           $stockOut = $rawStockout[0]->qty;
                                           } else {
                                           $stockOut = 0; 
                                           }
                                   ...
                                   @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $purchas_data->supplier->supplier_name ?? ''}}</td>
                                        <td>{{ $purchas_data->invoice }}</td>
                                        <td style="text-align:center">{{ $purchas_data->warehose->factory_name ?? ''}}</td>
                                        <td style="text-align:center">{{ $purchas_data->transport_vehicle }}</td>
                                        <td >{{ $purchas_data->product->product_name ?? ''}}</td>
