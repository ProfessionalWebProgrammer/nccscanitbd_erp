    ...
    <div class="content-wrapper" >
        ...
        <div class="content px-4 ">
          <div class="row pt-3">
            ...
            <div class="container" style="max-width: 1475px !important;" id="contentbody">


              <div class="row pt-2">
                  	<div class="col-md-4 text-left">
                      <h5 class="text-uppercase font-weight-bold">Monthly Purchase Report</h5>
                      <p>{{$rmonth}} - {{ $cyear}}</p>
                    ...
                    <table id="reporttable" class="table table-bordered table-striped table-fixed"
                        ...
                        <thead>
                            <tr class="table-header-fixt-top" style="font-size: 16px;font-weight: bold;color: rgb(0, 0, 0);top: -25px;">
                              ...
                              <th style="text-align:center;top: -35px; ">Pre Month</th>
                              <th style="text-align:center;top: -35px; ">Current Month</th>
                            ...
                            <tbody>
                                ...
                                @php
                                $wirehouses = DB::select('SELECT SUM(purchases.bill_quantity) as pqty  FROM `purchases`

                                WHERE purchases.month = "' . $pre_month . '" AND purchases.year = "' . $pre_year . '"  AND  purchases.product_id = "'.$data->product_id.'"
                                    AND purchases.raw_supplier_id = "'.$data->raw_supplier_id.'"
                               ');


                               $tpm += round($wirehouses[0]->pqty/1000,3);
                                ...
                                <tr>
                                    ...
                                    <td>{{round($wirehouses[0]->pqty/1000,3)}}</td>
                                ...
                                WHERE  purchases.wirehouse_id = "'.$wdata->id.'"
                                    ...
                                    AND purchases.raw_supplier_id = "'.$data->raw_supplier_id.'"
                                    AND purchases.month = "'.$rmonth.'"
                                    AND purchases.year = "'.$cyear.'" ');

                                    $wvalue[$key] += $w1[0]->total_qty/1000;
                                        @endphp
                                    <th>{{round($w1[0]->total_qty/1000,3)}}</th>
                                    @endforeach
