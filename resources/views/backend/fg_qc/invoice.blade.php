        ...
        <div class="content px-4 ">
            <div class="container-fluid bg-white p-5">
              <div class="row">
				...
				@php
                $pName = DB::table('sales_products')->where('id',$data->product_id)->value('product_name');
								$warehouse = DB::table('factories')->where('id',$data->warehouse_id)->value('factory_name');
                @endphp
                <div class="col-md-12 mt-4">
                  <div class="row">
                    <div class="col-md-8 h6" style="line-height:24px;">
                     {{-- <span>Chalan no: {{$data->chalan_no}}</span></br> --}}
                  	  <span>Warehouse Name: {{$warehouse}}</span></br>
                    {{--  <span>Contact No: {{$supplier->phone}}</span></br> --}}
                      <span>Date: {{ date("F d, Y") }}</span></br></br>
              			<span class="h5">Batch No: {{$data->batch_no}}</span></br>
