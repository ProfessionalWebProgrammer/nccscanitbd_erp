...
@section('content')
    ...
    <div class="content-wrapper">
      ...
      <form class="floating-labels m-t-40" action="{{route('fgQualityControl.update', $data->id)}}" method="POST">
          @csrf
      ...
      <div class="content px-4 ">
          <div class="container" style="background:#ffffff; padding:0px 40px;min-height:85vh">
              <div class="pt-3 text-center">
                  <h4 class="font-weight-bolder text-uppercase text-danger">F G Quality Control Create</h4>
                  <hr width="33%">
              </div>
            <div class="row pt-3">
                    <div class="col-md-12">
                        <div class="row mb-3">
                          ...
                          <div class="col-md-3">
                                <div class="form-group row">
                                <label class=" col-form-label col-md-4 pl-3"> Batch No:</label>
                                 <select name="batch_no" class="form-control select2 col-md-8 " required>
                                        <option value=""> Select Batch No </option>
                                         @foreach ($products as $item)
                                         <option value="{{$item->batch_id}}" @if($data->batch_no == $item->batch_id) selected  @else  @endif>{{$item->batch_id}}</option>
                                         @endforeach
                                 </select>
                                 </div>
                          </div>
                          <div class="col-md-3">
                                <div class="form-group row">
                                      <label class=" col-form-label col-md-4 pl-3"> Product Name:</label>
                                       <select name="product_id" class="form-control select2 col-md-8 " >
                                              <option value="">== Seleter Product ==</option>
                                               @foreach ($products as $item)
                                              <option value="{{$item->prouct_id}}"  @if($data->product_id == $item->prouct_id) selected @else  @endif >{{$item->name}}</option>
                                              @endforeach
                                       </select>
                                 </div>
                          </div>
                          <div class="col-md-3">
                             ...
                             <div class="col-md-4">
                                   <div class="form-group row">
                                        <label class=" col-form-label col-md-5 pl-3"> Select Warehouse:</label>
                                          <select name="warehouse_id" class="form-control select2 col-md-7 " required>
                                                 <option value="">== Select Warehouse ==</option>
                                                  @foreach ($warehouse as $item)
                                                  <option value="{{$item->id}}" @if($data->warehouse_id == $item->id) selected  @else  @endif>{{$item->factory_name}}</option>
                                                  @endforeach
                                          </select>
            ...
            @php
            $all = DB::table('qc_details')->where('item_type',1)->where('qc_id',$data->id)->get();
            @endphp
            <div class="row mt-3">
...
@endforeach
   ...
   */
           ...
           //alert(id);
           $.ajax({
                       url: '{{ url('purchase/fgQualitycontrol/getParameter/value/') }}/' + id,
                       type: "GET",
                       dataType: 'json',
