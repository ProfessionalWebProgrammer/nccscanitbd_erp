                      ...
                      <thead>
                        ...
                        <tbody>
                            ...
                            @foreach ($wirehousedata as $key => $wdata)
                               ...
                               $pre_stocktotal = DB::select('SELECT SUM(purchases.total_payable_amount) as amount,SUM(purchases.inventory_receive) as srcv FROM `purchases`
                              ...
                              $return = DB::select('SELECT SUM(purchase_returns.return_quantity) as return_qty, SUM(purchase_returns.total_amount) as amount FROM `purchase_returns`
                                   WHERE purchase_returns.product_id="'.$pdata->id.'" and purchase_returns.wirehouse_id ="'.$wdata->id.'"
                        ...
                        <tfoot>
                            <tr style="background-color: rgba(255, 127, 80, 0.466);">
                               ...
                               {{-- <th>{{ number_format($gtotal_op,2) }}</th>
                                ...
                                @endif --}}
                                <th colspan="5" style="text-align:right;">{{ number_format($grandTotalStockOutAmount,2) }}</th>
...
$grandTotalStockAmount = 0;
            ...
            @foreach ($wirehousedata as $key => $wdata)
                ...
                @foreach ($bagProducts as $key => $pdata)
                    @php
...
@endif --}}
        ...
        <tfoot>
            ...
            <tr style="background-color: rgba(255, 127, 80, 1); font-size:16px!important;">
              <th>Total Purchase Stock</th>
              <th colspan="100%" style="text-align:right;" >{{ number_format(($gtotalAmount + $grandTotalAmount),2) }}</th>
            </tr>
        </tfoot>
