                      ...
                      <thead>
                        ...
                        <tbody>
                            ...
                            @foreach ($wirehousedata as $key => $wdata)
                               ...
                               /*$stocktotal = DB::select('SELECT SUM(purchases.inventory_amount) as amount, SUM(purchases.inventory_receive) as srcv, SUM(purchases.bill_quantity) as qty, SUM(purchases.inventory_amount) as value FROM `purchases`
                               ...
                               $pre_stocktotal = DB::select('SELECT SUM(purchases.inventory_amount) as amount,SUM(purchases.inventory_receive) as srcv FROM `purchases`
                                    JOIN `row_materials_products` ON purchases.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                              ...
                              JOIN `row_materials_products` ON purchase_returns.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                ...
                                JOIN `row_materials_products` ON purchase_transfers.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                and purchase_transfers.to_wirehouse_id ="'.$wdata->id.'" and purchase_transfers.product_id="'.$pdata->id.'"  and purchase_transfers.date BETWEEN "'.$fdate.'" and "'.$tdate.'"');
                                 $pre_transfer_to = DB::select('SELECT SUM(purchase_transfers.receive_qty) as transfers_qty FROM `purchase_transfers`
                                 JOIN `row_materials_products` ON purchase_transfers.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                 and purchase_transfers.to_wirehouse_id ="'.$wdata->id.'" and purchase_transfers.product_id="'.$pdata->id.'"  and purchase_transfers.date BETWEEN "'.$sdate.'" and "'.$pdate.'"');

                                $transfer_from = DB::select('SELECT SUM(purchase_transfers.qty) as transfers_qty FROM `purchase_transfers`
                                JOIN `row_materials_products` ON purchase_transfers.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                and purchase_transfers.from_wirehouse_id ="'.$wdata->id.'" and purchase_transfers.product_id="'.$pdata->id.'"  and purchase_transfers.date BETWEEN "'.$fdate.'" and "'.$tdate.'"');
                                 $pre_transfer_from = DB::select('SELECT SUM(purchase_transfers.qty) as transfers_qty FROM `purchase_transfers`
                                 JOIN `row_materials_products` ON purchase_transfers.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                 and purchase_transfers.from_wirehouse_id ="'.$wdata->id.'" and purchase_transfers.product_id="'.$pdata->id.'"  and purchase_transfers.date BETWEEN "'.$sdate.'" and "'.$pdate.'"');
                              ...
                              JOIN `row_materials_products` ON purchase_damages.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                              and purchase_damages.warehouse_id ="'.$wdata->id.'" and purchase_damages.product_id="'.$pdata->id.'"  and purchase_damages.date BETWEEN "'.$fdate.'" and "'.$tdate.'"');
                              $pre_damage = DB::select('SELECT SUM(purchase_damages.quantity) as quantity FROM `purchase_damages`
                              JOIN `row_materials_products` ON purchase_damages.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                              and purchase_damages.warehouse_id ="'.$wdata->id.'" and purchase_damages.product_id="'.$pdata->id.'"  and purchase_damages.date BETWEEN "'.$sdate.'" and "'.$pdate.'"');
    */
                            $stocktotal = DB::select('SELECT SUM(purchases.inventory_amount) as amount, SUM(purchases.inventory_receive) as srcv, SUM(purchases.inventory_receive) as qty, SUM(purchases.inventory_amount) as value FROM `purchases`
                                                JOIN `row_materials_products` ON purchases.product_id = row_materials_products.id  where row_materials_products.category_id = "'.$category[$i].'"
                                                and purchases.product_id="'.$pdata->id.'" and purchases.wirehouse_id ="'.$wdata->id.'"  and  purchases.date between "'.$fdate.'" and "'.$tdate.'" ');
                          ...
                          $gtotalBagBalance += $pdata->opening_balance - $stockoutBags;
                           ...
                           $openingValue = $pdata->opening_balance*$pdata->rate;
                          	 ...
                          	 $openingbalance = $pdata->opening_balance + $pre_stocktotal[0]->srcv + $pre_transfer_to[0]->transfers_qty - $pre_return[0]->return_qty- $pre_damage[0]->quantity  - $pre_transfer_from[0]->transfers_qty - $pre_stock_out[0]->stockout;
                          	 $openingValue = $pdata->opening_balance*$pdata->rate + $pre_stocktotal[0]->amount + ($pre_transfer_to[0]->transfers_qty * $rate) - ($pre_return[0]->return_qty  * $rate) - ( $pre_damage[0]->quantity  * $rate)  - ( $pre_transfer_from[0]->transfers_qty  * $rate) -  $pre_stock_out[0]->amount;
                          				...
                          				$clsingbalance = $openingbalance + $stocktotal[0]->srcv - $return[0]->return_qty + $transfer_to[0]->transfers_qty - $damage[0]->quantity - $transfer_from[0]->transfers_qty - $stock_out[0]->stockout - $stockoutBags- $otherRawStockOut;
                          				} else {
                                    ...
                                    @endphp
                                        ...
                                        <tr>
                                          ...
                                          {{-- <td><a href="" data-toggle="modal" data-target="#modalPurchaseStock" >{{ $pdata->product_name }}</a></td> --}}
                                            ...
                                            <td>{{ number_format($stocktotal[0]->srcv, 2) }} @if($stocktotal[0]->srcv) {{$pdata->unit}} @else  @endif </td>
