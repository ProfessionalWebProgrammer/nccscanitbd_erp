    ...
    <div class="row">
        <div class="col-md-5" style="border-right: 2px solid black">
            ...
            <div class="content px-4 ">
                ...
                <form class="floating-labels m-t-40" action="{{ Route('sales.daily.incentive.store') }}" method="POST">
                    @csrf

                    <div class="">
                        <div class="pt-3 text-center">
                            <h4 class="font-weight-bolder text-uppercase">Daily Incentive Set</h4>
                            <hr width="33%">
                        </div>
                        ...
                        <div class="row pt-4">
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <input type="number" name="achive_commission" required class="form-control" step="any" placeholder="Achive Commision">
                            </div>
                            <div class="form-group col-md-12">
                                <label class="col-form-label">Description:</label>
                                <textarea name="description" class="form-control" id="" cols="30" rows="5" placeholder="Daily Incentive Description"></textarea>
                            </div>
                ...
                <div class="">
                    <div class="py-4">
                        <div class="text-center">
                            <h5 class="text-uppercase font-weight-bold">Daily Incentive List</h5>
                            <hr>
                        </div>
                        <table id="example5" class="table table-bordered table-striped" style="font-size: 15px;">
                            ...
                            @foreach ($data as $item)
                              @php 
                              $name = DB::table('sales_products')->where('id', $item->product_id)->value('product_name');
                              $cat = DB::table('sales_categories')->where('id', $item->category_id)->value('category_name');
                              @endphp
                                <tr>
                                    <td class="align-middle">{{ $loop->iteration }}</td>
                                    <td class="text-left align-middle">{{ $name }} </td>
									<td class="text-left align-middle">{{ $cat }} </td>
                                    <td class="text-center font-weight-bold align-middle">{{ $item->min_target }}-{{$item->max_target}} </td>
                                    <td class="text-center align-middle">{{ $item->achive_commision }} /Kg </td>
                                    <td class="text-left align-middle">{{ $item->description }} </td>
                                    <td class="text-center align-middle">
                                        <a class="btn btn-xs  salesedit" style="background-color: #66BB6A" href="{{route('sales.daily.incentive.edit',$item->id)}}"
...
<div class="modal fade" id="delete">
    <div class="modal-dialog">
        <div class="modal-content bg-danger">
            ...
            <form action="{{route('sales.daily.incentive.delete')}}" method="POST">
                {{ method_field('delete') }}
                @csrf
