                            ...
                            @endphp
                                ...
                                @php
                                   $startdate = '2023-09-30';
                                    ...
                                    $damage = \App\Models\SalesDamage::select(DB::raw('SUM(amount) as amount'),DB::raw('SUM(quantity) as qty'))->join('sales_products as s', 'sales_damages.product_id','=','s.id')->where('sales_damages.product_id',$all_products->id)->where('s.category_id',$category[$i])->whereBetween('date',[$fdate,$tdate])->first();
                                    $opdamage = \App\Models\SalesDamage::select(DB::raw('SUM(amount) as amount'),DB::raw('SUM(quantity) as qty'))->join('sales_products as s', 'sales_damages.product_id','=','s.id')->where('sales_damages.product_id',$all_products->id)->where('s.category_id',$category[$i])->whereBetween('date',[$startdate,$fdate2])->first();
                                    ...
                                    $dataItemOP = \App\Models\SalesProduct::select('product_name','opening_balance')->where('id',$all_products->id)->where('category_id',$category[$i])->first();
									 									$product_name = $dataItemOP->product_name ?? '';
																			...
																			*/
																				$opblnce = ($openingstock[0]->quantity+$optransfer_to + $all_products->opening_balance + $opreturnp[0]->qty)-($opsales[0]->qty+$optransfer_from+$opdamage->qty);
                                        $clb = ($opblnce+$todaystock[0]->quantity+$transfer_to + $returnp[0]->qty  ) - ($sales[0]->qty+$transfer_from+$damage->qty);
...
//dd($opblnce);
                                      ...
                                      $value = ($all_products->opening_balance*$all_products->rate)   + $allStock[0]->amount - $stockOutAmount - ($damage->amount + $opdamage->amount);
                                ...
                                <tr style="font-size: 12x;"><!--  -->
                                    ...
                                    <td>{{($sales[0]->qty) * $all_products->product_weight}} @if($sales[0]->qty) {{$all_products->weightUnit->unit_name ?? ''}} @else  @endif</td>
                                    <td>{{$sales[0]->qty}} @if($sales[0]->qty) {{$all_products->unit->unit_name ?? ''}} @else  @endif</td>
                                    <td>@if(($sales[0]->qty) > 0){{number_format(($thisMonthStockOutAmount/($sales[0]->qty)),2)}} @else  @endif</td>
                                    ...
                                    <td>{{number_format(($damage->qty * $all_products->product_weight), 2)}} @if($damage->qty) {{$all_products->weightUnit->unit_name ?? ''}} @else  @endif</td>
                                    <td>{{number_format($damage->qty, 2)}} @if($damage->qty) {{$all_products->unit->unit_name ?? ''}} @else  @endif</td>
                                    ...
                                    <td>{{number_format($clb, 2)}} @if($clb) {{$all_products->unit->unit_name ?? ''}} @else  @endif</td>
																		...
																		{{-- <td style="text-align:right;">@if($clb > 0) {{number_format($value,2)}} @else  0.00  @endif</td> --}}
                            ...
                            {{--    @if($totalPackingConsumption)
                                ...
                                @endif --}}
	                                   ...
	                                   {{-- <td colspan="9" style="text-align:right;">{{number_format($subStockOutAmount,2)}}</td> --}}
	                                    <td colspan="4" style="text-align:right;">{{number_format($subOpeningAmount,2)}}</td>
	                                    <td colspan="6" style="text-align:right;"></td>
																			<td colspan="14" style="text-align:right;">{{number_format($subTotalAmount,2)}}</td>
	                            	</tr>
																@endfor
																<tr>
																	<td colspan="2">Packing Amount : </td>
																	<td colspan="100%" style="text-align:right;"> {{number_format(($totalPackingConsumption + $totalPackingReturnCost - $totalPackingCost),2)}} </td>
																</tr>
                         </tbody>
                           ...
                           <tfoot>
                                  ...
                                  {{--  <td colspan="9" style="text-align:right;">{{number_format($grandTotalStockAmount,2)}}</td> --}}
                                    ...
                                    <td colspan="6" style="text-align:right;"></td>
																		<td colspan="14" style="text-align:right;">{{number_format((($gtotal_value + $totalPackingConsumption + $totalPackingReturnCost) - $totalPackingCost),2)}}</td>
                            </tr>
