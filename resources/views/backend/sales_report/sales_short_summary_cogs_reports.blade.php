          ...
          <div class="row pt-3">
            ...
            <div class="container-fluid" style="background:#ffffff; padding:0px 40px;min-height:85vh" id="contentbody">
                   ...
                   <div class="row pt-2">
                      ...
                      <h5 class="text-uppercase font-weight-bold">Sales Category Wise Cogs Report</h5>
                    ...
                    <table  id="reporttable" class="table table-bordered table-striped table-fixed" style="font-size: 15px;">
                            ...
                            @foreach ($category as $data)
                                ...
                                @foreach ($product_data as $product)
                                    @php
                                        ...
                                        $cogs = $sales_product->rate * $product->total_qty;
                                        $net_Sale_amount = $product->grand_total - $product->grand_total_discount_amount;
                                        
                                        $sub_t_discount_v += $product->grand_total_discount_amount;
                                        $sub_t_net_sale_v += $net_Sale_amount;
                                        $sub_t_cogs_v += $cogs;
                                        $sub_t_oh_v = 0;
                                        $sub_t_cogs_with_ho_v += $cogs + $o_h;
                                        $sub_t_gp_v += $net_Sale_amount - $cogs;
                                        ...
                                        $grnd_t_cogs_v += $cogs;
                                        $grnd_t_oh_v = 0;
                                        $grnd_t_cogs_with_ho_v += $cogs + $o_h;
                                        $grnd_t_gp_v += $net_Sale_amount - $cogs;
                                    ...
                                    <tr>
                                        ...
                                        <td align="right">{{ number_format($product->grand_total_discount_amount, 2)}}</td>
                                        <td align="right">{{ number_format($net_Sale_amount, 2) }}</td>
                                        <td align="right">{{ number_format($cogs, 2) }}</td>
                                        <td align="right">{{ number_format($o_h, 2) }}</td>
                                        <td align="right">{{ number_format($cogs + $o_h, 2) }}</td>
                                        <td align="right" title="GP">{{ number_format($net_Sale_amount - $cogs, 2) }}</td>
                                        <td align="right" title="GP%"></td>
                                    </tr>
                                @endforeach
                                <tr style="background-color: #fdc6964f;">
                                    ...
                                    <th  style="text-align:right">{{ number_format($sub_t_cogs_v, 2) }}</th>
                                    <th  style="text-align:right">{{ number_format($sub_t_oh_v, 2) }}</th>
                                    <th  style="text-align:right">{{ number_format($sub_t_cogs_with_ho_v, 2) }}</th>
                                    <th  style="text-align:right">{{ number_format($sub_t_gp_v, 2) }}</th>
                        ...
                        <tfoot>
                            <tr style="background-color: #80a5087c">
                                ...
                                <th colspan="4" style="text-align:right">{{ number_format($grnd_tvalue, 2) }}</th>
                                ...
                                <th  style="text-align:right">{{ number_format($grnd_t_cogs_v, 2) }}</th>
                                <th  style="text-align:right">{{ number_format($grnd_t_oh_v, 2) }}</th>
                                <th  style="text-align:right">{{ number_format($grnd_t_cogs_with_ho_v, 2) }}</th>
                                <th  style="text-align:right">{{ number_format($grnd_t_gp_v, 2) }}</th>
                                <th  style="text-align:right"></th>
                            </tr>
                        </tfoot>
                    </table>
