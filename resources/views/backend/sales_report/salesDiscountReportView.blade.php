                    ...
                    <div class="text-center pt-3" style="  ">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4 text-left align-middle pl-3">
                              	<h5 style="font-weight: 800;">Sales Discount Ledger</h5>
                        ...
                        <table class="table table-bordered p-2" style="font-size: 13px;font-weight: 600; ">
                            ...
                            <tbody style="font-size: 12px;">
                                ...
                                @php
                                $details = \App\Models\SalesLedger::where('vendor_id', $data->vendor_id)->where('discount_amount', '>', 0)->whereBetween('ledger_date',[$fdate, $tdate])->get();
                                $subTotalAmount = 0;
                                @endphp

                                <tr style="font-size: 15px; font-weight:500; ">
                                    <td colspan="100%" class="text-left"><a href="{{url('/deler/index')}}?vendor_id={{$data->vendor_id}}" target="_blank"  style="color:#082d91 !important;"> {{ $data->dealer->d_s_name ?? ''}}</a>  </td>
                                </tr>
                                @foreach ($details as $key => $item)

                                @php
                                $subTotalAmount += $item->discount_amount;
                                $gTotalAmount  += $item->discount_amount;
                                @endphp

                                <tr style="font-size: 13px;">
                                    <td>{{ date('d-m-Y', strtotime($item->ledger_date)) }}</td>

                                    <td>  <a href="{{URL::to('/sales/invoice/'.$item->invoice)}}" target="_blank" > {{$item->invoice}} </a></td>
                                ...
                                <tr style="font-size: 15px;color:#fff; font-weight:600;background-color:#b88c15; ">
                                    <td colspan="3" class="text-left"  style="padding: 12px;"> Grand Total </td>
