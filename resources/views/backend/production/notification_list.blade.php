...
@section('header_menu')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ URL('/stockin/notification') }}" class=" btn btn-success btn-sm mr-2 mt-1">Crate New Notification</a>
    </li>
@endsection
@section('content')
                        ...
                        <tbody>
                            ...
                            @foreach ($notifications as $key=> $data)
                                <tr>
                                    ...
                                    <td class="text-left">{{ $data->factory_name }}</td>
                                    <td class="text-left">{{ $data->product_name }}</td>
                                    <td class="text-left">{{ $data->minimum_qty }} </td>
    ...
    <div class="modal fade" id="delete">
            <div class="modal-dialog">
                <div class="modal-content bg-danger">
                    ...
                    <form action="{{ route('stockin.notification.delete')}}" method="POST">
                        {{ method_field('delete') }}
                        @csrf
