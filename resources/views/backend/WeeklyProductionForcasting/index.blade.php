        ...
        <div class="content px-4 ">
            <div class="container-fluid" style="background:#ffffff; padding:0px 40px;min-height:85vh;min-width: 100% !important;">
              <div class="row pt-3">
                  ...
                  <a href="{{route('wpf.create')}}" class=" btn btn-success mr-2">Create</a>
              </div>
                </div>
                <div class="text-center pt-3">
                    <h3 class="text-uppercase font-weight-bold">Naba Crop Care</h3>
                    <p>Head office, Rajshahi, Bangladesh</p>
                    <h5>Weekly Production Forcasting List</h5>
                </div>
                            ...
                            @foreach ($data as $key => $val)
                                <tr>
                                    ...
                                    <td class="tdDate">{{ date('M d, y', strtotime($val->issue_date)) }}</td>
									                  <td class="tdDate">{{ date('M d, y', strtotime($val->f_date)) }} - {{ date('M d, y', strtotime($val->t_date)) }}</td>
                                    <td>{{ $val->total}}</td>
                                    <td>{{$val->note}}</td>
                                    <td>
                                        {{-- <a href="" target="_blank" class="btn btn-xs btn-info purchaseedit"><span
                                                class="fa fa-edit"></span></a> --}}
                                      <a href="{{route('wpf.view',$val->id)}}" class="btn btn-primary btn-xs"><span
      ...
      <div class="modal fade" id="delete">
        <div class="modal-dialog">
            <div class="modal-content bg-danger">
                ...
                <form action="{{ route('wpf.delete') }}" method="POST">
                    {{ method_field('delete') }}
                    @csrf
